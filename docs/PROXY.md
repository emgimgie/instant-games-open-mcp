...
- **会话隔离** - 每个用户的数据完全隔离
- **进度透传** - 自动转发下游 MCP Server 的 progress 通知
...
6. **转发进度** - 当客户端传入 `progressToken` 时，转发 `notifications/progress`
7. **返回结果** - 返回结果给 AI Agent
...
- `X-TapTap-Custom-Fields`: 业务自定义字段 JSON（可选，如 `{"team":"studio-a","env":"staging"}`）
...
- `tenant.custom_fields` - 业务自定义字段（可选，`Record<string, string>`，透传到 Server）
...
function startMCPProxy(user: User, project: Project, macToken: MacToken) {
  ...
  const proxy = spawn(
    'npx',
    ['-y', '-p', '@mikoto_zero/minigame-open-mcp@latest', 'taptap-mcp-proxy', configJson],
    {
