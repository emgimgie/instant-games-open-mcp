...
export interface ToolRegistration {
  ...
  handler: (args: any, context: ToolContext, extra?: ToolCallExtra) => Promise<string>;
...
handler: async (args: { page: number }, context: ResolvedContext, extra?: ToolCallExtra) => {
  // 可选：向客户端上报进度
  await extra?.sendProgress?.(10, 100, 'loading leaderboard data');
